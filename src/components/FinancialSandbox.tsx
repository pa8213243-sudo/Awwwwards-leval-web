...
export const FinancialSandbox: React.FC = () => {
  ...
  return (
    <div id="sandbox" className="relative space-y-6 w-full py-8 md:py-12 border-b border-white/10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 overflow-hidden">
      {/* CONTEXTUAL PROFESSIONAL BACKGROUND PHOTO */}
      <SectionBackgroundLayer sectionKey="sandbox" opacity={0.4} />

      {/* ELEGANT FIXED VERTICAL PROGRESS BAR ON RIGHT EDGE */}
      <VerticalSectionProgressBar targetId="sandbox" accentColor="#E0533C" label="LAB" sectionCode="04" />
      ...
      <ScrollReveal delay={0.1}>
        <div className="w-full bg-[#0D0D12] border border-white/20 p-5 md:p-6 rounded-none shadow-2xl relative overflow-hidden text-white select-none">
          {/* Background Graphic Grid */}
          <div className="absolute inset-0 brutalist-grid opacity-30 pointer-events-none" />

          {/* HEADER BAR */}
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/10 pb-4 mb-5">
            <div>
              ...
              <h3 className="font-serif text-xl md:text-2xl font-bold tracking-tight uppercase">
                Live DCF Sensitivity & Enterprise Valuation Sandbox
              </h3>
              <p className="text-xs text-white/70 max-w-xl font-sans mt-0.5">
            ...
            <div className="flex items-center gap-2.5">
              <button
                onClick={handleReset}
                className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/20 font-mono text-[11px] uppercase tracking-wider text-white/70 hover:text-white flex items-center gap-1.5 rounded-none transition-colors cursor-pointer"
              >
                <RotateCcw className="w-3 h-3" />
                <span>RESET MODEL</span>
              </button>

              <div className="px-3 py-1 bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 font-mono text-[10px] font-bold uppercase rounded-none">
                IMA COMPLIANT
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 space-y-4 bg-black/50 border border-white/10 p-4 rounded-none">
          <div className="text-[11px] font-mono uppercase tracking-wider text-gray-400 font-bold border-b border-white/10 pb-2 flex items-center justify-between">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-gray-300">Base Revenue (LTM)</span>
              <span className="font-bold text-white">${revenueBase}M USD</span>
            </div>
            <input
              type="range"
              min={10}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-gray-300">5-Yr Revenue CAGR</span>
              <span className="font-bold text-emerald-400">+{growthRate}% YoY</span>
            </div>
            <input
              type="range"
              min={2}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-gray-300">Target EBITDA Margin</span>
              <span className="font-bold text-blue-400">{ebitdaMargin}%</span>
            </div>
            <input
              type="range"
              min={5}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-gray-300">WACC Discount Rate</span>
              <span className="font-bold text-amber-400">{waccDiscount}%</span>
            </div>
            <input
              type="range"
              min={6}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-gray-300">Exit Multiple (EV/EBITDA)</span>
              <span className="font-bold text-purple-400">{exitMultiple}x</span>
            </div>
            <input
              type="range"
              min={6}
        ...
        <div className="lg:col-span-7 flex flex-col justify-between space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <div className="p-3.5 bg-black/60 border border-[#E0533C]/40 rounded-none">
              <div className="text-[9px] font-mono text-gray-400 uppercase font-bold mb-0.5">
                ENTERPRISE VALUE (EV)
              </div>
            ...
            <div className="p-3.5 bg-black/60 border border-emerald-500/40 rounded-none">
              <div className="text-[9px] font-mono text-gray-400 uppercase font-bold mb-0.5">
                IMPLIED SHARE PRICE
              </div>
            ...
            <div className="p-3.5 bg-black/60 border border-blue-500/40 rounded-none">
              <div className="text-[9px] font-mono text-gray-400 uppercase font-bold mb-0.5">
                PV OF TERMINAL VALUE
              </div>
              <div className="font-serif text-xl sm:text-2xl font-bold text-blue-400">
          ...
          <div className="border border-white/10 bg-black/40 rounded-none p-3.5 overflow-x-auto">
            <div className="text-[11px] font-mono uppercase tracking-wider text-gray-300 font-bold mb-2.5 flex items-center justify-between">
              <span>5-YEAR PRO-FORMA CASH FLOW PROJECTIONS ($M USD)</span>
              <span className="text-[10px] text-emerald-400 font-semibold">DYNAMIC RECALCULATION</span>
            </div>

            <table className="w-full text-left font-mono text-xs">
              <thead>
                <tr className="border-b border-white/10 text-[10px] text-gray-400">
                  <th className="pb-2 font-bold">METRIC / LINE ITEM</th>
                  {valuation.forecasts.map((f, i) => (
                    <th key={i} className="pb-2 text-right font-bold text-white">{f.year}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 text-[11px]">
                <tr>
                  <td className="py-1.5 text-gray-300 font-medium">Revenue</td>
                  {valuation.forecasts.map((f, i) => (
                    <td key={i} className="py-1.5 text-right font-bold text-white">${f.revenue}M</td>
                  ))}
                </tr>
                <tr>
                  <td className="py-1.5 text-gray-300 font-medium">EBITDA ({ebitdaMargin}%)</td>
                  {valuation.forecasts.map((f, i) => (
                    <td key={i} className="py-1.5 text-right text-blue-400 font-semibold">${f.ebitda}M</td>
                  ))}
                </tr>
                <tr>
                  <td className="py-1.5 text-gray-300 font-medium">Free Cash Flow (FCF)</td>
                  {valuation.forecasts.map((f, i) => (
                    <td key={i} className="py-1.5 text-right text-emerald-400 font-semibold">${f.fcf}M</td>
                  ))}
                </tr>
                <tr className="bg-white/5 font-bold">
                  <td className="py-1.5 text-white">Discounted PV @ {waccDiscount}%</td>
                  {valuation.forecasts.map((f, i) => (
          ...
          <div className="p-3 bg-white/5 border border-white/10 rounded-none font-mono text-[11px] text-gray-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
