...
&& [Department_Access] IN 
  ...
  return (
    ...
    >
      ...
      <SectionBackgroundLayer sectionKey="dashboards" opacity={0.4} />

      {/* ELEGANT FIXED VERTICAL PROGRESS BAR ON RIGHT EDGE */}
      <VerticalSectionProgressBar targetId="dashboards" accentColor="#3B82F6" label="TELEMETRY" sectionCode="06" />
      ...
      <div className="max-w-7xl mx-auto w-full my-auto relative z-10 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-6 pt-4">
        ...
        {/* LEFT VERTICAL SECTION SPINE (Vertical Typography & Telemetry Pipeline) */}
        <div className="hidden md:flex flex-col items-center justify-between py-4 px-2.5 bg-black/85 border border-blue-500/35 backdrop-blur-md flex-shrink-0 z-20 shadow-2xl">
          ...
          {/* Vertical Stage Stepper Nodes (01 -> 03) */}
          <div className="flex flex-col items-center gap-2 my-auto py-2">
            <div className="w-[1px] h-6 bg-gradient-to-b from-transparent via-blue-500/50 to-blue-500" />
            {stages.map((st, idx) => (
              <button
                key={st.id}
                ...
                className={`w-6 h-6 flex items-center justify-center font-mono text-[10px] font-bold border transition-all cursor-pointer ${
                  activeStage === idx
                    ? 'bg-blue-600 text-white border-blue-400 shadow-[0_0_12px_rgba(59,130,246,0.7)] scale-110'
                    : 'bg-black/60 text-white/50 border-white/20 hover:border-blue-400/50 hover:text-white'
                }`}
                title={st.title}
              >
                0{idx + 1}
            ...
            <div className="w-[1px] h-6 bg-gradient-to-b from-blue-500 via-blue-500/50 to-transparent" />
          </div>
        ...
        <div className="flex-1 flex flex-col justify-between space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
            ...
            <div className="flex items-center gap-2 self-end sm:self-center">
              <button
                ...
                className={`px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-wider border transition-all cursor-pointer flex items-center gap-1.5 rounded-none shadow-md ${
                  showHeatmap
                    ? 'bg-rose-600 text-white border-rose-400 shadow-[0_0_15px_rgba(225,29,72,0.6)] animate-pulse'
                    : 'bg-blue-950/80 text-blue-300 border-blue-500/50 hover:bg-blue-900/60'
                }`}
                title="Toggle Data Density Heatmap Overlay to view high-engagement metric zones"
                data-cursor="HEATMAP"
              >
                <Flame className={`w-3.5 h-3.5 ${showHeatmap ? 'text-white' : 'text-rose-400'}`} />
                <span>{showHeatmap ? 'HEATMAP ON' : 'DATA DENSITY HEATMAP'}</span>
              </button>

              <div className="font-mono text-xs text-blue-400 font-bold px-2.5 py-1 bg-blue-950/60 border border-blue-500/30">
                STAGE 0{activeStage + 1} / 0{totalStages} ACTIVE
              </div>
            </div>
          </div>

          {/* THREE STAGE SELECTOR TABS WITH GSAP FLIP */}
          <div ref={tabsContainerRef} className="grid grid-cols-1 md:grid-cols-3 gap-2.5 sm:gap-3">
            {stages.map((st, idx) => {
              ...
              return (
                <div
                  key={idx}
                  onClick={() => handleSelectStage(idx)}
                  className={`p-3 sm:p-3.5 border transition-all duration-300 cursor-pointer flex items-center gap-3 ${
                    isActive
                      ? 'bg-[#12121A] border-2 border-blue-400 text-white shadow-2xl scale-[1.01] ring-1 ring-blue-400/40'
                      : 'bg-black/50 border-white/10 text-white/60 hover:bg-white/5 hover:text-white'
                  }`}
                >
                  <div className="relative w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 overflow-hidden border border-white/20 bg-black">
                    <LazyRenderMedia
                      src={st.image}
                      alt={st.title}
                  ...
                  <div className="flex flex-col justify-between h-full space-y-0.5 flex-1 min-w-0">
                    <div className="flex items-center justify-between font-mono text-[9px] text-blue-400 font-bold">
                      <span className="truncate">{st.badge}</span>
                      {isActive && <CheckCircle2 className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />}
                    </div>
                    <h4 className="font-mono text-xs font-bold uppercase tracking-wider text-white line-clamp-1">{st.title}</h4>
                    <p className="text-[10px] font-sans text-white/50 line-clamp-1 leading-tight">{st.detail}</p>
                  </div>
                </div>
              );
            })}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center bg-[#101016]/95 backdrop-blur-xl border border-white/15 p-4 sm:p-6 shadow-2xl"
            >
              ...
              <div className="lg:col-span-7 space-y-3">
                <div className="relative overflow-hidden border border-white/20 shadow-2xl">
                  ...
                  <div className="p-2.5 bg-black/80 border-t border-white/10 flex items-center justify-between">
                    <h3 className="font-serif text-lg sm:text-xl text-white font-bold">{currentStageData.title}</h3>
                    {showHeatmap && (
                      <span className="text-[10px] font-mono text-rose-400 font-bold flex items-center gap-1 uppercase">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {currentStageData.kpis.map((kpi, kIdx) => (
                    <div key={kIdx} className="bg-white/5 border border-white/10 p-2.5 font-mono">
                      <div className="text-[9px] text-white/50 font-bold uppercase">{kpi.label}</div>
                      <div className="text-sm sm:text-base font-bold text-white mt-0.5">{kpi.value}</div>
                      <div className="text-[9px] text-emerald-400 font-bold">{kpi.delta}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="lg:col-span-5 space-y-3">
                <div className="bg-[#0A0A0E] text-white border border-blue-500/30 p-3 sm:p-4 space-y-2 font-mono shadow-xl">
                  ...
                  <div className="text-[10px] text-white/70 font-sans border-t border-white/10 pt-2 flex items-start gap-2">
